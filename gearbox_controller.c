 ...
 * VERSION: 4.0 FINAL (Competition / Production Edition)
...
#define CAN_PROFILE_FLASH_ADDR  0x0800F800
#define ERROR_LOG_ADDR          0x0800F000
...
#define COOLANT_TEMP_ADC_CH     ADC_CHANNEL_8
#define IGNITION_PIN            GPIO_PIN_9

#define COLD_ENGINE_RPM_LIMIT   4000
#define NORMAL_ENGINE_RPM_LIMIT 7000
#define SPORT_RPM_LIMIT         7200
...
typedef struct {
    uint16_t rpm_id; uint8_t rpm_offset; uint8_t rpm_length; uint8_t rpm_factor; uint8_t rpm_big_endian;
    uint16_t speed_id; uint8_t speed_offset; uint8_t speed_length; uint8_t speed_factor; uint8_t speed_big_endian;
    uint16_t throttle_id; uint8_t throttle_offset; uint8_t throttle_length; uint8_t throttle_factor;
    uint16_t brake_id; uint8_t brake_offset; uint8_t brake_bit;
    uint16_t front_speed_id; uint8_t front_speed_offset; uint8_t front_speed_length;
    uint16_t rear_speed_id; uint8_t rear_speed_offset; uint8_t rear_speed_length;
} CanProfile;
...
/* ==================== 4. ENUMERATIONS ==================== */
typedef enum { CAL_STATE_IDLE=0, CAL_STATE_CLUTCH, CAL_STATE_AXIS_X, CAL_STATE_AXIS_Y, CAL_STATE_BACKLASH, CAL_STATE_GEAR_POSITIONS, CAL_STATE_SAVE, CAL_STATE_DONE, CAL_STATE_ERROR } CalibrationState;
typedef enum { CLUTCH_DISENGAGE, CLUTCH_ENGAGE, CLUTCH_TO_BITE } ClutchAction;
typedef enum { PRIMARY_AS5600, BACKUP_POTENTIOMETER, SENSOR_FAULT } ActiveClutchSensor;
typedef enum { DRIVE_MODE_COMFORT=0, DRIVE_MODE_NORMAL=1, DRIVE_MODE_SPORT=2, DRIVE_MODE_WINTER=3 } DriveMode;
typedef enum { AWD_MODE_2WD=0, AWD_MODE_AUTO=1, AWD_MODE_LOCK=2, AWD_MODE_LOW=3 } AwdMode;

/* ==================== 5. ERROR CODES ==================== */
#define ERR_NONE 0, ERR_CURRENT_OVER_X 1, ERR_CURRENT_OVER_Y 2, ERR_CURRENT_OVER_CLUTCH 3, ERR_LIMIT_SWITCH_STUCK 4, ERR_STEPPER_STALL 5, ERR_CLUTCH_TIMEOUT 6, ERR_AS5600_MISSING 7, ERR_GEAR_NOT_FOUND 8, ERR_SHIFT_TIMEOUT 9, ERR_WATCHDOG_RESET 10, ERR_EMERGENCY_BUTTON 11

/* ==================== 6. GLOBAL VARIABLES ==================== */
I2C_HandleTypeDef hi2c1; SPI_HandleTypeDef hspi2; UART_HandleTypeDef huart1, huart3;
CAN_HandleTypeDef hcan1; ADC_HandleTypeDef hadc1; TIM_HandleTypeDef htim2, htim4; IWDG_HandleTypeDef hiwdg;

CalibrationData cal_data = {0}; ClutchCalibration clutch_cal = {0};
AxisCalibration cal_X = {0}, cal_Y = {0}; BackupSensorCalibration backup_cal = {0}; CanProfile current_profile = {0};

volatile uint8_t current_gear = 0, current_shift_target_gear = 0, shifting_in_progress = 0;
volatile DriveMode current_drive_mode = DRIVE_MODE_NORMAL;
volatile CalibrationState cal_state = CAL_STATE_IDLE;
volatile uint8_t calibration_running = 0, calibration_error = 0, calibration_success = 0, cal_in_progress = 0, abort_calibration = 0;
volatile uint8_t system_sleeping = 0, limp_mode_active = 0, is_parked = 0;
volatile uint32_t can_loss_timer = 0, engine_off_timer = 0;
volatile bool as5600_comm_ok = false;

volatile uint8_t ignition_on = 0;
volatile uint16_t coolant_temp_raw = 0;
volatile uint8_t coolant_temp_celsius = 20;
volatile uint16_t engine_rpm_limit = 7000;

volatile uint8_t speed_limiter_enabled = 0;
volatile uint16_t speed_limit_kmh = 50;
volatile uint8_t speed_limit_mph_mode = 0;
volatile uint8_t cruise_control_active = 0;
volatile uint16_t cruise_target_speed = 0;

int32_t current_x_steps = 0, current_y_steps = 0;

volatile uint16_t engine_rpm = 0;
volatile uint8_t vehicle_speed_kmh = 0, throttle_percent = 0, brake_pressed = 0;
volatile uint8_t shift_paddle_up = 0, shift_paddle_down = 0;
uint8_t can_profile_valid = 0;

uint16_t filtered_rpm = 0;
uint8_t filtered_throttle = 0;

volatile uint8_t limit_x_left_triggered = 0, limit_x_right_triggered = 0;
volatile uint8_t limit_y_front_triggered = 0, limit_y_back_triggered = 0;
...
uint8_t error_index = 0, error_count = 0;

volatile uint32_t last_activity_time = 0, auto_calib_timer = 0;

volatile AwdMode current_awd_mode = AWD_MODE_2WD;
volatile uint8_t awd_engaged = 0;
volatile uint32_t awd_engagement_time = 0;
AwdConfig awd_config = { .awd_enable_pin = GPIO_PIN_4, .awd_engage_delay_ms = 200, .awd_auto_speed_diff = 10, .awd_lock_timeout_s = 30 };

volatile uint16_t front_wheel_speed_raw = 0, rear_wheel_speed_raw = 0;

/* Shift Maps */
static const ShiftMapEntry shift_map_comfort[] = { {0,3500,1100}, {20,3800,1200}, {40,4200,1400}, {60,4800,1700}, {80,5200,2000}, {100,5800,2400} };
static const ShiftMapEntry shift_map_normal[]  = { {0,4000,1400}, {20,4500,1700}, {40,5000,2100}, {60,5500,2600}, {80,6000,3200}, {100,6500,3800} };
static const ShiftMapEntry shift_map_sport[]   = { {0,4500,1800}, {20,5000,2000}, {40,5500,2300}, {60,6000,2600}, {80,6500,3000}, {100,7000,3400} };
static const ShiftMapEntry shift_map_winter[]  = { {0,1800,900},  {20,2000,1000}, {40,2400,1200}, {60,2800,1400}, {80,3000,1600}, {100,3200,1800} };
...
static volatile uint32_t tacho_period_us = 0, tacho_last_capture = 0;

/* ==================== 7. FUNCTION PROTOTYPES ==================== */
void SystemClock_Config(void); void Error_Handler(void); static void DWT_Init(void); static void sleep_us(uint32_t us);
static void MX_GPIO_Init(void); static void MX_I2C1_Init(void); static void MX_SPI2_Init(void);
static void MX_USART1_UART_Init(void); static void MX_USART3_UART_Init(void); static void MX_CAN1_Init(void);
static void MX_ADC1_Init(void); static void MX_TIM2_Init(void); static void MX_TIM4_Init(void);
void IWDG_Init(void); void IWDG_Refresh(void);

void stepper_enable_x(void); void stepper_disable_x(void); void stepper_enable_y(void); void stepper_disable_y(void);
void stepper_set_direction_x(uint8_t dir); void stepper_set_direction_y(uint8_t dir);
void stepper_step_x(void); void stepper_step_y(void);
void stepper_move_x(uint32_t steps, uint32_t delay_us); void stepper_move_y(uint32_t steps, uint32_t delay_us);
void axis_move_relative(uint8_t axis, int32_t steps, uint32_t step_delay_us);

uint16_t AS5600_ReadAngle(void); uint8_t AS5600_Init(void);
void clutch_set_direction(uint8_t dir); void clutch_set_speed(uint8_t duty); void clutch_stop(void);
uint8_t clutch_is_endstop_triggered(void); uint16_t clutch_find_disengaged(void);
uint16_t clutch_find_bite_point(uint16_t dis, uint16_t eng); uint8_t clutch_calibrate(void);
uint16_t clutch_get_disengaged_angle(void); uint16_t clutch_get_engaged_angle(void);
uint16_t clutch_get_bite_point(void); uint16_t clutch_get_current_angle(void);
...
uint16_t backup_to_as5600_angle(uint16_t raw); void check_sensor_consistency(void);

uint8_t axis_calibrate_x(void); uint8_t axis_calibrate_y(void); void run_full_calibration(void); void learn_gear_positions(void);
uint8_t flash_save_calibration(CalibrationData* data); uint8_t flash_load_calibration(CalibrationData* data);
static void flash_unlock(void); static void flash_lock(void); static void flash_erase_page(uint32_t addr);
static uint16_t calculate_crc16(uint8_t* data, uint32_t len);

uint8_t is_shift_safe_winter(uint8_t from, uint8_t to);
void move_to_neutral(void); void move_to_gear_position_advanced(uint8_t gear, uint8_t axis);
uint8_t shift_gear_advanced(uint8_t target); uint8_t shift_gear_limp_safe(uint8_t target);
uint16_t get_adaptive_downshift_hold_time(uint8_t from, uint8_t to);

uint16_t get_rpm_universal(void); uint8_t get_speed_universal(void); void filter_data(void);
void auto_shift_task(void); void manual_shift_task(void);
uint8_t auto_scan_can(void); void save_can_profile(void); void load_can_profile(void); void apply_vehicle_adaptation(void);

void safety_init(void); void emergency_power_off(void); void emergency_power_on(void); void emergency_stop(void);
void current_monitor_task(void); uint8_t diagnostic_check(void);
void log_error(uint8_t code, uint8_t severity, uint16_t data); void save_error_log_to_flash(void);
float get_current(uint8_t motor_idx); void calibrate_current_offsets(void); void check_critical_failures(void);
void enter_limp_mode(const char* reason); void exit_limp_mode(void);

void activity_reset_timer(void); void idle_monitor_task(void); void power_clutch_off(void); void power_clutch_on(void);
DriveParameters get_current_drive_params(void); void set_drive_mode(DriveMode mode);
void set_drive_mode_by_name(const char* mode_str); uint8_t get_start_gear(void); void start_moving(void);
void execute_parking_mode(void); void exit_parking_mode(void); void parking_monitor_task(void);

void awd_init(void); void awd_set_engaged(uint8_t engage); uint8_t get_wheel_speed_diff(void);
void awd_auto_task(void); void awd_set_mode(AwdMode mode);

void uart_send_string(char* str); void uart_clear_buffer(void); void uart_command_handler(char* cmd);
void bt_send_string(char* str); void send_status_via_bt(void); void process_bt_command(void);
void visualize_axes(void);

void led_set_pattern(uint8_t pattern); void main_loop(void);

void read_coolant_temperature(void); void update_engine_rpm_limit(void); void handle_speed_limiter(void);
void handle_cruise_control(void); void process_ignition_key(void); void cascade_zero_throttle_downshift(void);

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin);
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef* hcan);
void USART1_IRQHandler(void); void USART3_IRQHandler(void);
void TIM4_IRQHandler(void); void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef* htim);

/* ==========================================================================
 * PART 2: HARDWARE INITIALIZATION & LOW-LEVEL ACTUATOR CONTROL
 * ========================================================================== */
...
static void MX_GPIO_Init(void) {
    ...
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_2 | GPIO_PIN_5, GPIO_PIN_SET); // Start disabled
    
    // Clutch H-bridge direction
    ...
    // Limit switches and buttons (falling edge interrupts)
...
static void MX_USART3_UART_Init(void) {
    ...
    AFIO->MAPR |= AFIO_MAPR_USART3_REMAP; // Remap to PC10/PC11
...
static void MX_TIM2_Init(void) {
    ...
    htim2.Init.Period = 999;    // 100 kHz / 1000 = 100 Hz PWM
...
static void MX_TIM4_Init(void) {
    ...
    htim4.Instance = TIM4;
    htim4.Init.Prescaler = 71; // 72 MHz / 72 = 1 MHz (1 µs resolution)
...
/* --- Stepper and Clutch Low-Level Control --- */
...
void clutch_set_speed(uint8_t duty) {
    uint32_t pulse = (uint32_t)duty * 10;
...
/* ==========================================================================
 * PART 3: AXIS CALIBRATION, FLASH STORAGE, SHIFTING LOGIC & SAFETY
 * ========================================================================== */

/* --- Axis Calibration and Backlash Measurement --- */
...
uint8_t flash_save_calibration(CalibrationData* data) {
    if ((uint32_t)data + sizeof(CalibrationData) > 0x08010000) return 0;
...
/* --- LED Patterns and Full Calibration --- */
...
/* --- UART Helpers and Gear Learning --- */
...
/* --- Advanced Shifting Logic (FIXED) --- */

uint16_t get_engine_rpm(void) { return get_rpm_universal(); }
uint8_t get_vehicle_speed(void) { return get_speed_universal(); }

/**
 * @brief Check if the requested shift is safe.
 * FIX: Removed the erroneous (rpm > p.max_rpm_shift) check that blocked upshifts.
 * Now shifts are governed solely by the shift_map, allowing normal operation 
 * up to the engine's natural redline.
 */
uint8_t is_shift_safe_winter(uint8_t from, uint8_t to) {
...
float get_current(uint8_t idx) {
    ...
    float amps = volts / 0.185f; // ACS712-30A: 185 mV/A
...
/* ==========================================================================
 * PART 4: CAN COMMUNICATION, TASKS, POWER MANAGEMENT, ADVANCED LOGIC & MAIN
 * ========================================================================== */

/* --- CAN Communication and Shifting Tasks --- */
...
void process_bt_command(void) {
    ...
    switch (c) {
        case '1': case '2': case '3': case '4': case '5':
            if (can) shift_gear_limp_safe(c - '0'); else bt_send_string("Shift blocked\r\n"); break;
        case 'N': case 'n':
            if (can) shift_gear_limp_safe(0); else bt_send_string("Shift blocked\r\n"); break;
        case 'R': case 'r':
            if (can && get_vehicle_speed() < 5 && current_gear == 0) shift_gear_limp_safe(6);
            else bt_send_string("Reverse only when stopped in N\r\n"); break;
        case 'A': case 'a':
            current_drive_mode = DRIVE_MODE_COMFORT; bt_send_string("Mode: AUTO\r\n"); led_set_pattern(2); break;
        case 'M': case 'm':
            current_drive_mode = DRIVE_MODE_NORMAL; bt_send_string("Mode: MANUAL\r\n"); led_set_pattern(0); break;
        case 'S': case 's':
            send_status_via_bt(); break;
        default:
...
void uart_command_handler(char* cmd) {
    ...
    if (strcmp(cmd, "help") == 0) {
        ...
        char b[UART_MSG_BUF_SIZE];
        uart_send_string("\r\n--- STATUS ---\r\n");
        snprintf(b, sizeof(b), "Mode:%s Gear:%s\r\n", (current_drive_mode != DRIVE_MODE_NORMAL) ? "AUTO" : "MANUAL", (current_gear == 0) ? "N" : (current_gear == 6) ? "R" : gear_names[current_gear]);
        uart_send_string(b);
        ...
        uart_send_string(b);
        snprintf(b, sizeof(b), "Calibrated:%s Limp:%s Sleep:%s\r\n", cal_data.calibrated ? "YES" : "NO", limp_mode_active ? "ACTIVE" : "OFF", system_sleeping ? "YES" : "NO");
        uart_send_string(b);
        ...
        if (calibration_running || shifting_in_progress || !cal_data.calibrated) uart_send_string("Complete basic calibration first!\r\n");
        else learn_gear_positions();
    } else if (strncmp(cmd, "mode ", 5) == 0) {
        set_drive_mode_by_name(cmd + 5);
        ...
        else if (gc == 'R' || gc == 'r' || gc == '6') {
            if (get_vehicle_speed() < 5 && current_gear == 0) t = 6;
            else { uart_send_string("Reverse only when stopped and in N\r\n"); return; }
        } else { uart_send_string("Invalid gear\r\n"); return; }
        if (!shift_gear_limp_safe(t)) uart_send_string("Shift blocked\r\n");
    } else if (strcmp(cmd, "can_scan") == 0) {
        can_profile_valid = 0;
        if (auto_scan_can()) apply_vehicle_adaptation();
        ...
        uart_send_string(b);
    } else if (strcmp(cmd, "fault can") == 0) {
        enter_limp_mode("SIMULATED CAN LOSS");
    } else if (strcmp(cmd, "clear fault") == 0) {
        exit_limp_mode();
        ...
        char b[UART_MSG_BUF_SIZE];
        snprintf(b, sizeof(b), "Active:%s AS5600:%d Backup raw:%d Mapped:%d\r\n", (active_sensor == PRIMARY_AS5600) ? "AS5600" : "POT", AS5600_ReadAngle(), clutch_backup_sensor_raw, backup_to_as5600_angle(clutch_backup_sensor_raw));
        uart_send_string(b);
        ...
        uart_send_string("Unknown. Type 'help'.\r\n");
    }
}
...
void idle_monitor_task(void) {
    ...
    last = now;
    if (!system_sleeping && !shifting_in_progress && !calibration_running && (now - last_activity_time) >= IDLE_TIMEOUT_MS) {
        system_enter_sleep();
    }
}
...
void set_drive_mode(DriveMode m) {
    if (m == current_drive_mode || shifting_in_progress) return;
    current_drive_mode = m;
    const char* n[] = { "COMFORT", "NORMAL", "SPORT", "WINTER" };
    char msg[64];
    snprintf(msg, sizeof(msg), ">>> DRIVE MODE: %s <<<\r\n", n[m]);
    uart_send_string(msg);
    bt_send_string(msg);
    activity_reset_timer();
}
...
void start_moving(void) {
    activity_reset_timer();
    uint8_t t = get_start_gear();
    if (current_gear != 0) shift_gear_limp_safe(0);
    HAL_Delay(100);
    if (get_speed_universal() < 5) shift_gear_limp_safe(t);
    else uart_send_string("Start blocked: speed high.\r\n");
}
...
void awd_set_mode(AwdMode m) {
    if (m == current_awd_mode) return;
    if ((m == AWD_MODE_LOCK || m == AWD_MODE_LOW) && get_speed_universal() > 30) {
        uart_send_string("AWD blocked: speed high\r\n");
        return;
    }
    current_awd_mode = m;
    const char* n[] = { "2WD", "AUTO", "LOCK", "LOW" };
    char msg[64];
    snprintf(msg, sizeof(msg), ">>> AWD: %s <<<\r\n", n[m]);
    uart_send_string(msg);
    bt_send_string(msg);
}

/* --- Analog Tachometer Callbacks --- */
...
/* --- Redundant Clutch Sensor (Final Bits) --- */
...
void main_loop(void) {
    ...
    if (now - t500 >= 500) {
        t500 = now;
        IWDG_Refresh(); // CRITICAL: Keep watchdog alive
        if (cal_data.calibrated && !calibration_running) {
            diagnostic_check();
            read_clutch_backup_sensor();
...
int main(void) {
    ...
    // CRITICAL FIX: Initialize watchdog timer
    IWDG_Init();

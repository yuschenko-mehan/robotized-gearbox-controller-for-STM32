/**
 * @file    gearbox_controller.c
 * @brief   Adaptive Robotized Manual Transmission Controller for STM32F103C8T6
 * @version 3.0
 * @author  (Your Name)
 * @date    2026
 ...
 * (X axis: shift row selection, Y axis: gear engagement) and a DC motor for
 * the clutch actuator. It reads vehicle data from a CAN bus (500 kbps),
 * implements automatic and manual shifting modes, provides comprehensive
 * safety features (current monitoring, emergency stop, limp mode),
 * redundant clutch position sensing (AS5600 magnetic encoder + backup
 * potentiometer), power saving sleep mode, and an interactive UART/Bluetooth
 * command interface.
 *
 ...
 *   final creep to fully engaged position is done at a slow speed (20% PWM)
 *   to eliminate jerks. The aggressive "dump" mode has been removed.
 * - The incorrect RPM limit that previously blocked upshifts has been
 *   completely removed from is_shift_safe_winter().
 ...
 * - Full documentation in Doxygen style.
 */
 
/* ==========================================================================
 * 1. INCLUDES
 * ========================================================================== */
#include "main.h"                // STM32 HAL generated file (includes chip definitions)
#include "stm32f1xx_hal.h"       // Hardware abstraction layer for STM32F1 family
#include <stdbool.h>             // Boolean type for cleaner logic
...
/* Optional SD card support – comment out if not used */
// #define USE_SD_CARD
...
/* ==========================================================================
 * 2. CRITICAL FLASH MEMORY ADDRESSES (last 4 KB of 64 KB Flash)
 * ========================================================================== */
#define CALIB_FLASH_ADDR        0x0800FC00   // Calibration data (1 KB page)
#define CAN_PROFILE_FLASH_ADDR  0x0800F800   // CAN message profile (1 KB page)
#define ERROR_LOG_ADDR          0x0800F000   // Error log (4 KB page)

/* ==========================================================================
 * 3. SYSTEM CONSTANTS
 * ========================================================================== */
#define CALIB_MAGIC             0xA5C3F0F0   // Magic number to validate calibration data
#define ERROR_LOG_MAX_ENTRIES   50           // Maximum number of error records in RAM buffer
#define IDLE_TIMEOUT_MS         5000         // Milliseconds of inactivity before sleep
#define SENSOR_TOLERANCE        100          // Max allowed difference between AS5600 and backup
...
#define BACKOFF_STEPS           50           // Steps to back off from endstop after hit
#define CLUTCH_TIMEOUT_MS       5000         // Max time allowed for clutch calibration
#define UART_MSG_BUF_SIZE       64           // Size of local string buffers

/* ==========================================================================
 * 4. DATA STRUCTURES (packed for Flash storage)
 * ========================================================================== */
 ...
 */
typedef struct {
    uint16_t angle_disengaged;   ///< Angle when clutch is fully pulled against endstop
    uint16_t angle_engaged;      ///< Angle when clutch is fully engaged (pedal pressed)
    uint16_t angle_bite_point;   ///< Calculated point where clutch starts to transmit torque
    uint8_t  calibrated;         ///< Flag: 1 if calibration was successful
} ClutchCalibration;
 ...
 */
typedef struct {
    int32_t left_limit;          ///< Absolute step count when left/front limit switch triggered
    int32_t right_limit;         ///< Absolute step count when right/back limit switch triggered
    int32_t home_offset;         ///< Step count corresponding to neutral position
    int32_t backlash;            ///< Measured mechanical play in steps
    uint8_t calibrated;          ///< Flag: 1 if axis calibration is valid
} AxisCalibration;
...
typedef struct __attribute__((packed)) {
    ...
    uint16_t crc;                ///< CRC-16 of the entire structure (excluding this field)
    ...
    /* Gear positions: 7 gears (0=Neutral, 1..5, 6=Reverse) each with X and Y steps */
    int32_t  gear_positions[7][2];
    
    uint8_t  calibrated;         ///< Global flag: 1 if all gears have been taught
} CalibrationData;

/**
 * @brief Backup clutch sensor (analog potentiometer) calibration.
 * Maps raw ADC values (0-4095) to AS5600 angle range using linear scale.
 */
typedef struct {
    uint16_t raw_min;            ///< ADC reading when clutch is fully disengaged
    uint16_t raw_max;            ///< ADC reading when clutch is fully engaged
    float scale;                 ///< Multiplier: (AS5600_range) / (backup_range)
    uint8_t calibrated;          ///< Flag: 1 if backup sensor is calibrated
} BackupSensorCalibration;

/**
 * @brief CAN message profile – describes where to find RPM, speed, throttle, brake.
 * Allows the controller to adapt to different vehicle CAN buses.
 */
typedef struct {
    uint16_t rpm_id;             ///< CAN ID containing engine RPM
    uint8_t rpm_offset;          ///< Byte offset within CAN data where RPM starts
    uint8_t rpm_length;          ///< Number of bytes (1 or 2)
    uint8_t rpm_factor;          ///< Multiplier to convert raw value to RPM (e.g., 4)
    uint8_t rpm_big_endian;      ///< 1 = big-endian (Motorola), 0 = little-endian (Intel)
 ...
 */
typedef struct {
    uint32_t timestamp;          ///< HAL_GetTick() value at error occurrence
    uint8_t  error_code;         ///< One of the ERR_* defines
    uint8_t  severity;           ///< 0=Info, 1=Warning, 2=Critical
    uint16_t data;               ///< Additional context (e.g., current value*10)
} ErrorLogEntry;

/**
 * @brief Shift map entry – defines RPM thresholds for a given throttle position.
 * Used in automatic mode. Each drive mode has its own table.
 */
typedef struct {
...
/**
 * @brief Drive mode parameters – adjust clutch speed, hold time, etc.
 */
typedef struct {
    uint8_t clutch_disengage_speed;   ///< PWM duty cycle (0-100) when disengaging clutch
    uint8_t clutch_engage_speed;      ///< PWM duty cycle when engaging clutch (to bite point)
    uint16_t clutch_hold_time_ms;     ///< Time clutch stays fully disengaged during shift
    uint16_t shift_delay_us;          ///< Delay between stepper motor steps (lower = faster)
    uint8_t use_bite_point;           ///< Always 1 in v3.0 (smooth engagement)
    uint8_t throttle_blip;            ///< Reserved for future automatic rev‑matching
...
typedef struct {
    ...
    uint8_t awd_lock_timeout_s;       ///< Maximum seconds in LOCK mode before reverting to AUTO
} AwdConfig;
...
typedef enum {
    ...
    CLUTCH_ENGAGE,       ///< Release clutch smoothly (always uses bite point)
    CLUTCH_TO_BITE       ///< Move only to the bite point (for starting from standstill)
} ClutchAction;
...
/** Driving modes – index into params_table[] and shift map tables */
typedef enum {
...
/* ==========================================================================
 * 7. GLOBAL VARIABLES (handles and volatile state)
 * ========================================================================== */
...
volatile uint8_t current_shift_target_gear = 0;        // Temporary during shift
volatile uint8_t shifting_in_progress = 0;              // 1 = gear change ongoing
...
volatile uint8_t limit_x_left_triggered = 0;   // PB2
volatile uint8_t limit_x_right_triggered = 0;  // PB3
volatile uint8_t limit_y_front_triggered = 0;  // PB10
volatile uint8_t limit_y_back_triggered = 0;   // PB11
volatile uint8_t clutch_endstop_triggered = 0; // PB1
...
/**
 * @brief Returns the shift map and its size for the current drive mode.
...
/* Drive parameters – use_bite_point is forced to 1 for smooth engagement (even in SPORT) */
...
/* Known CAN profiles (Ford, VAG, Toyota, BMW) – for auto‑detection */
...
/* ==========================================================================
 * 8. FUNCTION PROTOTYPES (hardware initialization)
 * ========================================================================== */
...
/* ==========================================================================
 * 9. HARDWARE INITIALIZATION (complete implementations)
 * ========================================================================== */

/**
 * @brief System Clock Configuration – 72 MHz using external 8 MHz HSE.
 */
void SystemClock_Config(void) {
 ...
 * @param us microseconds (max ~1 second at 72 MHz)
 */
static void sleep_us(uint32_t us) {
...
static void MX_GPIO_Init(void) {
    ...
    /* Clutch H‑bridge direction pins */
...
/**
 * @brief I2C1 initialization for AS5600 (100 kHz).
 */
static void MX_I2C1_Init(void) {
    hi2c1.Instance = I2C1;
...
/**
 * @brief USART1 initialization for PC diagnostic console (115200 baud, 8N1).
 */
static void MX_USART1_UART_Init(void) {
...
/**
 * @brief USART3 initialization for Bluetooth HM-10 (9600 baud, 8N1, remapped to PC10/PC11).
 */
static void MX_USART3_UART_Init(void) {
...
/**
 * @brief CAN1 initialization – 500 kbps, accepts all standard IDs.
 */
static void MX_CAN1_Init(void) {
    hcan1.Instance = CAN1;
...
/**
 * @brief ADC1 initialization – scans 4 channels (PC0, PC1, PC2, PC4).
 */
static void MX_ADC1_Init(void) {
    ...
    HAL_ADC_ConfigChannel(&hadc1, &sConfig);
    sConfig.Channel = ADC_CHANNEL_14; sConfig.Rank = 4;
    HAL_ADC_ConfigChannel(&hadc1, &sConfig);
}

/**
 * @brief TIM2 initialization for clutch PWM (100 Hz, Channel 1).
 */
static void MX_TIM2_Init(void) {
    htim2.Instance = TIM2;
...
/**
 * @brief TIM4 initialization for analog tachometer input capture (PB6).
 */
static void MX_TIM4_Init(void) {
    ...
    htim4.Instance = TIM4;
    htim4.Init.Prescaler = 72 - 1;                  // 1 MHz (1 µs/tick)
...
/**
 * @brief Independent Watchdog initialization (1 sec timeout).
 */
void IWDG_Init(void) {
    hiwdg.Instance = IWDG;
...
/* ==========================================================================
 * 10. STEPPER AND CLUTCH LOW‑LEVEL CONTROL
 * ========================================================================== */
...
/* ==========================================================================
 * 12. AXIS CALIBRATION AND BACKLASH
 * ========================================================================== */
...
/* ==========================================================================
 * 14. LED PATTERNS AND FULL CALIBRATION MACHINE
 * ========================================================================== */
    ...
    while (cal_state != CAL_STATE_DONE && cal_state != CAL_STATE_ERROR && abort_calibration == 0) {
        ...
        switch (cal_state) {
            case CAL_STATE_CLUTCH:
                ...
                } else cal_state = CAL_STATE_ERROR;
                break;
...
/* ==========================================================================
 * 15. UART, GEAR LEARNING AND SHIFTING LOGIC
 * ========================================================================== */
...
/* ---- Shifting logic (advanced) with smooth clutch ---- */
uint16_t get_engine_rpm(void) { return get_rpm_universal(); }
uint8_t get_vehicle_speed(void) { return get_speed_universal(); }

/* is_shift_safe_winter – checks only reverse at speed and winter 1st gear (NO RPM limit) */
uint8_t is_shift_safe_winter(uint8_t from, uint8_t to) {
 ...
 * For CLUTCH_ENGAGE, we always use the bite point and then creep slowly.
 */
void clutch_control_advanced(ClutchAction a) {
    ...
    switch (a) {
        ...
        case CLUTCH_ENGAGE:
            ...
            clutch_stop();
            HAL_Delay(300);
            // 2. Slowly creep to fully engaged position – eliminates jerks
            clutch_set_direction(1);
            clutch_set_speed(20);
            start = HAL_GetTick();
...
/* ==========================================================================
 * 16. CURRENT MONITORING AND SAFETY (partial – key functions shown)
 * ========================================================================== */
...
float get_current(uint8_t idx) {
    ...
    float volts = diff * (3.3f / 4096.0f); float amps = volts / 0.185f;
...
/* ==========================================================================
 * 17. CAN, AUTO/MANUAL SHIFTING, BLUETOOTH, UART COMMANDS
 * ========================================================================== */
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef* hcan) {
...
void auto_shift_task(void) {
    ...
    for (uint8_t i = 0; i < map_size; i++) {
        if (filtered_throttle <= map[i].throttle) { e = &map[i]; break; }
    }
...
void manual_shift_task(void) {
    static uint32_t last = 0; uint32_t now = HAL_GetTick();
...
/* ==========================================================================
 * 18. REDUNDANT CLUTCH SENSOR AND ADVANCED FEATURES
 * ========================================================================== */
void read_clutch_backup_sensor(void) {
...
/* ==========================================================================
 * 19. POWER MANAGEMENT, PARKING, AWD, CAN PROFILING, ANALOG TACHO
 * ========================================================================== */
...
static void power_actuators_off(void) { power_clutch_off(); }
...
void awd_auto_task(void) {
    static uint32_t last = 0; uint32_t now = HAL_GetTick(); if (now - last < 100) return; last = now; uint8_t diff = get_wheel_speed_diff(); uint8_t spd = get_speed_universal(); switch (current_awd_mode) {
        ...
        case AWD_MODE_AUTO: if (diff >= awd_config.awd_auto_speed_diff) { if (!awd_engaged) awd_set_engaged(1); } else if (awd_engaged && spd > 15) awd_set_engaged(0); break;
...
/* ==========================================================================
 * 20. ANALOG TACHOMETER (TIM4 input capture)
 * ========================================================================== */
...
/* ==========================================================================
 * 21. MAIN LOOP AND POWER MANAGEMENT
 * ========================================================================== */
void main_loop(void) {
